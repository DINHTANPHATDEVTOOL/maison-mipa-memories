...
--   - Adds database trigger on bookings to authoritatively create delivery intent on SHOOT_COMPLETED.
...
-- Strictly service_role only. No client role (authenticated or anon) can ever SELECT or write.
...
-- Only active ADMIN or Root Owner can manage oauth states
...
-- Management full operational control on raw table
...
-- Explicitly DROP any direct SELECT policies for CUSTOMER and STAFF from raw table.
-- Customers and staff MUST use public.get_booking_delivery_secure(p_booking_id)
-- so internal tokens/permission IDs/credentials are never exposed to clients.
...
-- ------------------------------------------------------------------------------
-- 4. Server-Authoritative Trigger: SHOOT_COMPLETED creates durable delivery intent
-- ------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.handle_booking_shoot_completed()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
DROP TRIGGER IF EXISTS trg_booking_shoot_completed ON public.bookings;
CREATE TRIGGER trg_booking_shoot_completed
AFTER INSERT OR UPDATE OF booking_status ON public.bookings
FOR EACH ROW
EXECUTE FUNCTION public.handle_booking_shoot_completed();

-- ------------------------------------------------------------------------------
-- 5. Trigger: Sync booking_deliveries to bookings compatibility fields
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 6. Helper Function: Role-Safe Secure Delivery Reader (Blocker 9)
-- ------------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_caller_role = 'STAFF' THEN
    ...
    -- Strip customer permission id & share email from staff view
    RETURN jsonb_build_object(
      'id', v_delivery.id,
      'booking_id', v_delivery.booking_id,
  ...
  IF v_caller_role = 'CUSTOMER' THEN
    ...
    IF v_delivery.status != 'READY_FOR_CUSTOMER' THEN
      -- Customer sees delivery status, but NEVER drive URL or internal IDs
      RETURN jsonb_build_object(
        'id', v_delivery.id,
        'booking_id', v_delivery.booking_id,
        'status', v_delivery.status,
    ...
    -- When READY_FOR_CUSTOMER, return customer-facing drive url, masking all internal IDs
    RETURN jsonb_build_object(
      'id', v_delivery.id,
      'booking_id', v_delivery.booking_id,
      'status', v_delivery.status,
...
-- ------------------------------------------------------------------------------
-- 7. Locked Enqueue Function for Drive Delivery Ready (Blocker 10)
-- ------------------------------------------------------------------------------
...
BEGIN
  -- 1. Enforce that delivery record exists and status is strictly READY_FOR_CUSTOMER
...
-- Strictly REVOKE execute from authenticated / public. Allow service_role only (Blocker 10)
...
-- ------------------------------------------------------------------------------
-- 8. Safe Backfill (Blocker 15: Never backfill to READY_FOR_CUSTOMER without verified Google permission)
-- ------------------------------------------------------------------------------
INSERT INTO public.booking_deliveries (
  booking_id,
  provider,
