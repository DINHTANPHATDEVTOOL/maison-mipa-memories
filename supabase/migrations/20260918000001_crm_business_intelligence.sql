...
BEGIN
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      'serviceId', s.id,
      'serviceName', s.name,
      'requestsCount', COALESCE(COUNT(b.id), 0),
      'confirmedCount', COALESCE(COUNT(b.id) FILTER (WHERE b.booking_status IN ('CONFIRMED', 'CHECKED_IN', 'SHOOTING', 'SHOOT_COMPLETED', 'AWAITING_SELECTION', 'EDITING', 'READY_FOR_REVIEW', 'DELIVERED', 'COMPLETED')), 0),
      'completedCount', COALESCE(COUNT(b.id) FILTER (WHERE b.booking_status = 'COMPLETED'), 0),
      'confirmedBookingValue', COALESCE(SUM(b.total_amount) FILTER (WHERE b.booking_status IN ('CONFIRMED', 'CHECKED_IN', 'SHOOTING', 'SHOOT_COMPLETED', 'AWAITING_SELECTION', 'EDITING', 'READY_FOR_REVIEW', 'DELIVERED', 'COMPLETED')), 0),
      'conversionRate', CASE 
...
BEGIN
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      'conceptId', c.id,
      'conceptName', c.name,
      'timesSelected', COUNT(bc.id),
      'confirmedBookings', COUNT(bc.id) FILTER (WHERE b.booking_status IN ('CONFIRMED', 'CHECKED_IN', 'SHOOTING', 'SHOOT_COMPLETED', 'AWAITING_SELECTION', 'EDITING', 'READY_FOR_REVIEW', 'DELIVERED', 'COMPLETED')),
      'completedBookings', COUNT(bc.id) FILTER (WHERE b.booking_status = 'COMPLETED')
...
BEGIN
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      'studioId', sr.id,
      'studioName', sr.name,
      'bookedHours', ROUND(COALESCE(SUM(EXTRACT(EPOCH FROM (b.end_at - b.start_at)) / 3600), 0)::NUMERIC, 1),
      'operatingHoursTotal', ROUND((v_days * 12)::NUMERIC, 1), -- 12 hours/day (08:00 - 20:00)
      'utilizationPercent', ROUND(LEAST(100.0, (COALESCE(SUM(EXTRACT(EPOCH FROM (b.end_at - b.start_at)) / 3600), 0) / (v_days * 12)) * 100)::NUMERIC, 1)
    )
  ) INTO v_studios
