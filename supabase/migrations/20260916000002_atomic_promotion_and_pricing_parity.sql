-- ==============================================================================
-- Maison MIPA Memories - Database Migration
-- File: 20260916000002_atomic_promotion_and_pricing_parity.sql
--
-- Features:
-- 1. Hardens create_booking promotion usage limit against race conditions:
--    - Locks promotion row with FOR UPDATE before usage checks
--    - Null-safe usage_count verification: COALESCE(v_promo.usage_count, 0) >= v_promo.usage_limit
--    - Atomic usage_count increment: COALESCE(usage_count, 0) + 1
-- 2. Client/Server pricing parity:
--    - max_discount caps BOTH percentage and fixed-amount discounts
--    - Percentage discount precedence preserved if both present
-- 3. Preserves all authoritative constraints:
--    - active authenticated user status validation
--    - package/service association (service-specific OR universal)
--    - flexible cross-service concepts & concepts_count limit
--    - active studio validation & duration calculation with addons
--    - studio overlap lock
--    - audit trail
--    - jsonb return representation
-- ==============================================================================
...
BEGIN
  ...
  -- 3. Validate Concepts & Enforce Package concepts_count Limit (Optional, Cross-Service Allowed)
  ...
  -- 7. Calculate Server-Authoritative Pricing & Validate Promotion Voucher
  IF p_addon_ids IS NOT NULL AND array_length(p_addon_ids, 1) > 0 THEN
  ...
  IF p_voucher_code IS NOT NULL AND trim(p_voucher_code) != '' THEN
    -- Authoritative promotion row lock with FOR UPDATE before usage checks
    ...
    -- Parity with frontend calculatePricing(): max_discount caps BOTH percentage and fixed-amount discounts
