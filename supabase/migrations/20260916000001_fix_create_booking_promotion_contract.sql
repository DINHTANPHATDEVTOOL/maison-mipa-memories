...
-- File: 20260916000001_fix_create_booking_promotion_contract.sql
--
-- Features:
-- 1. Updates create_booking RPC to align with the authoritative promotions table schema:
--    - start_at, end_at (was starts_at, expires_at)
--    - usage_count, usage_limit (was times_used)
--    - min_order (was min_order_amount)
--    - discount_percent, discount_amount, max_discount (was discount_type, discount_value, max_discount_amount)
--    - applicable_service_id check against p_service_id
--    - INCREMENTS usage_count (NOT times_used)
-- 2. Preserves:
--    - active authenticated user validation
--    - package/service association
--    - flexible cross-service concepts & concepts_count package limit
--    - active studio validation & duration calculation with addons
--    - studio overlap lock
--    - audit trail
--    - return jsonb representation
-- ==============================================================================
...
BEGIN
  ...
  IF p_voucher_code IS NOT NULL AND trim(p_voucher_code) != '' THEN
    ...
    IF v_promo.discount_percent > 0 THEN
      ...
      IF v_promo.max_discount IS NOT NULL AND v_promo.max_discount > 0 AND v_discount_total > v_promo.max_discount THEN
    ...
    SET usage_count = usage_count + 1, updated_at = timezone('utc'::text, now())
    WHERE id = v_promo.id;
  END IF;
