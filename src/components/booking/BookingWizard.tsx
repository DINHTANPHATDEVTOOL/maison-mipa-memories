...
export const BookingWizard: React.FC<BookingWizardProps> = ({
  ...
  // Proceed from Step 5: Submit Consultation Request
  const handleSubmitConsultation = async () => {
    ...
    try {
      ...
      if (!isSupabaseConfigured()) {
        newBooking = createBookingInMemory(payload);
      } else {
        newBooking = await createBooking(payload);
      }
    ...
    <div className={presentation === 'PAGE' ? 'booking-wizard-page-wrapper' : 'modal-overlay'}>
      ...
      >
        ...
        <div key={step} className="booking-step-content" style={{ padding: '1.8rem', overflowY: 'auto', flex: 1 }}>
              ...
              <div style={{
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.8rem', color: 'var(--editorial-text-secondary)', marginTop: '0.2rem' }}>
                    Tiền cọc dự kiến (30%): <strong>{depositAmount.toLocaleString('vi-VN')}đ</strong>
                  </div>
                </div>
          ...
          {/* STEP 6: CONSULTATION REQUEST SUBMITTED SCREEN */}
            ...
            <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <span style={{ color: 'var(--editorial-text-muted)', fontSize: '0.8rem', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                    Mã đặt lịch
                  </span>
                  <strong style={{ fontWeight: 700, fontSize: '1.1rem', color: 'var(--editorial-brown)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem', fontSize: '0.9rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <strong>{createdBooking.customerName || customerName} ({customerPhone})</strong>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--editorial-text-secondary)' }}>Dịch vụ:</span>
                    <strong>{selectedService?.name || createdBooking.serviceName}</strong>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--editorial-text-secondary)' }}>Gói chụp:</span>
                    <strong>{selectedPackage?.name || createdBooking.packageName}</strong>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--editorial-text-secondary)' }}>Ngày chụp mong muốn:</span>
                    <strong>{selectedDate || createdBooking.bookingDate}</strong>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--editorial-text-secondary)' }}>Khung giờ mong muốn:</span>
                    <strong>{selectedTimeSlot || createdBooking.startTime} ({totalDurationMinutes} phút)</strong>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--editorial-text-secondary)' }}>Phòng / Studio:</span>
                    <strong>{selectedStudio?.name || createdBooking.studioName}</strong>
                  </div>
                </div>
                ...
                <div style={{
                  marginTop: '1.4rem',
                  paddingTop: '1rem',
                  borderTop: '1px dashed var(--editorial-divider)',
                  display: 'flex',
                  justifyContent: 'space-between',
                  alignItems: 'center',
                }}>
                  <div>
                    <span style={{ fontSize: '0.8rem', color: 'var(--editorial-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'block' }}>
                      Chi phí dự kiến
                    </span>
                    <span style={{ fontSize: '0.78rem', color: 'var(--editorial-brown-accent)' }}>
                      (Chưa bao gồm ưu đãi khi tư vấn)
                    </span>
                  </div>
                  <div style={{
                    fontFamily: 'var(--editorial-font-heading)',
          ...
          <div className="booking-wizard-footer" style={{
            ...
            <div style={{ display: 'flex', gap: '1.2rem', alignItems: 'center' }}>
              <span style={{ fontSize: '0.88rem', color: 'var(--editorial-brown)', fontWeight: 600 }}>
                Chi phí dự kiến: {totalAmount.toLocaleString('vi-VN')}đ
              </span>
              <button
                type="button"
                onClick={() => {
                ...
                aria-label={step === 5 ? 'Gửi yêu cầu tư vấn Tiếp theo' : 'Tiếp theo'}
                className="public-btn-primary"
                style={{ fontSize: '0.85rem', padding: '0.6rem 1.4rem' }}
              >
                {step === 5 ? (isSubmitting ? 'Đang gửi yêu cầu...' : 'Gửi yêu cầu tư vấn') : 'Tiếp theo'} <ChevronRight size={16} />
              </button>
            </div>
          </div>
        )}
